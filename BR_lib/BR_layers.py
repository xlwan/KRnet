...
#  return tf.keras.initializers.GlorotNormal(seed=23)
...
class flow_mapping(layers.Layer):
  ...
  def call(self, inputs, logdet=None, reverse=False):
    if not reverse:
      ...
      if self.n_bins > 0:# nolinear invertible mapping after affine coupling
      ...
      if self.n_bins > 0: # nonlinear invertible mapping before affine coupling
        i_split_at=self.n_split_at
        z1 = z[:,:i_split_at]
        z2 = z[:,i_split_at:]
...
class CDF_quadratic(layers.Layer):
    def __init__(self, name, n_bins, r=1.2, bound=30.0, beta=1e-5, **kwargs):
